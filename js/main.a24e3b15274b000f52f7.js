...
function addNew() {
  ...
  addNewButton.onclick = function () {
    ...
    document.querySelector('#alert').classList.remove("uk-transition-slide-right");
    document.querySelector('.close-loader .foreground').classList.add('close-loader-move');
    Object(_alertPopUp__WEBPACK_IMPORTED_MODULE_2__["default"])('Error'); //add-new-wrapper
    //itemCardNew.classList.remove("uk-transition-slide-right");
...
// import & alertPopUp('text error');
function alertPopUp(alertTxt) {
  var alertClose = document.querySelector('#alert-close');

  var removeCloseMove = () => {
    document.querySelector('.close-loader .foreground').classList.remove('close-loader-move');
  };
  ...
  textAlert.textContent = alertTxt; // закрыть алерт через 6 сек
    ...
    editItem.onclick = function () {
      ...
      this.onclick = function () {
        ...
        if (ksaNum == '') {
          ...
          localStorage[ksaNum] = JSON.stringify(dataArrToLocalStor);
          editClick();
        }
      }; // Del Click
...
function getLS(formatDate, date) {
  for (var ksaN in localStorage) {
    //Поиск по ключу
    var keysKsa = localStorage.getItem(ksaN); //let obj = JSON.parse(keysKsa);

    if (keysKsa !== null && keysKsa.length > 5) {
      //console.log(JSON.parse(keysKsa));
      // Key Value Object
      var keyObj = Object.values(JSON.parse(keysKsa)); // Вендор
      ...
      document.querySelector('#out').insertAdjacentHTML('afterbegin', outDivext);
    }
...
/******/ });
//# sourceMappingURL=main.a24e3b15274b000f52f7.js.map